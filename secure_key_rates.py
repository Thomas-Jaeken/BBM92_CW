...
class secure_key_rates:
    ...
    def __init__(self, name, d, t_delta, eff_A, eff_B, DC_A, DC_B, e_b, e_p, f=1.1, eps_tot=0.01, eps_EC=0.01, t_dead=0, loss_format='loss', custom=False,B0=1):
        self.f = f
        self.name = name
        ...
        for j in range(self.d):
            for k in range(self.d):
                ...
                result += self.__coincidence_window_loss__(x[0], j, k) * x[1] * self.__total_efficiency__(
                    self.efficiencies_A[j], x[1], j, k) * self.__total_efficiency__(self.efficiencies_B[k], x[1], j, k)
                # Contribution of accidental CC
                result += x[0] * (x[1]*self.__total_efficiency__(self.efficiencies_A[j], x[1], j, k)+self.dark_counts_A[j]) * (
                    x[1]*self.__total_efficiency__(self.efficiencies_B[k], x[1], j, k)+self.dark_counts_B[k]) / \
                    self.__dead_efficiency__(self.efficiencies_B[k], x[1], j, k) /  \
                    self.__dead_efficiency__(
                    self.efficiencies_A[j], x[1], j, k)
    ...
    def __coincidences_erroneous__(self, x, error_rate):
        ...
        for j in range(self.d):
            for k in range(self.d):
                if not j == k:
                    # the contribution of true CC
                    result += error_rate * self.__coincidence_window_loss__(x[0], j, k) * x[1] * self.__total_efficiency__(
                        self.efficiencies_A[j], x[1], j, k) * self.__total_efficiency__(self.efficiencies_B[k], x[1], j, k)
                    # Contribution of accidental CC
                    result += x[0] * (x[1]*self.__total_efficiency__(self.efficiencies_A[j], x[1], j, k)+self.dark_counts_A[j]) * (
                        x[1]*self.__total_efficiency__(self.efficiencies_B[k], x[1], j, k)+self.dark_counts_B[k]) / \
    ...
    def error_rate(self, x):
        return self.__coincidences_erroneous__(x, self.bit_error) / self.__coincidences_measured__(x)
    ...
    def optimize_performance(self, B0=1):
        ...
        result = minimize(
            obj, [1, B0], bounds=[(0.001, 10), (1e-9, 1e3)])
    ...
    def __S_bbm92__(self, E, pa0, pb1, N, epsilon, d=2):
        ''' The entropy is the only part that is protocol specific, this is for ekert protocol'''
        xi = 0
        pb = [pb1, 1 - pb1]
        for i in range(2):
            for j in range(2):
                m = 1/2 * (1 - pa0) * pb[j] * N
                xi += np.sqrt((2 * np.log(1 / epsilon) +
                              d * np.log(m + 1)) / m)

        return 1 - self.__binary_entropy__(E + xi)

    def __delta__(self, epsilon, n):
        return 7 * np.sqrt(np.log2(2 / epsilon) / n)

    def __leak__(self, E, n):
        return self.f * n * self.__binary_entropy__(E) + np.log2(2 / self.eps_EC)

    def __key_ratio_bbm92__(self, e_rate, eps_PE, eps_AD, eps_PA, pa0, pb1, N):
        n = pa0 * pb1 * N
        # is the error rate here the E = CCt/CCm or the e_pol????????
        return (n * (self.__S_bbm92__(e_rate, pa0, pb1, N, eps_PE) - self.__delta__(eps_AD, n)) - self.__leak__(e_rate, n) - 2 * np.log2(1 / eps_PA)) / N

    def __objective_finite__(self, x):
        q = 0.5
        CC_m = self.__coincidences_measured__(x[:2])
        E_b = self.__coincidences_erroneous__(x[:2], self.bit_error) / CC_m
        # does the ratio take bit_error or Eb???
        return - q * CC_m * self.__key_ratio_bbm92__(E_b, *x[2:], N=q * CC_m)

    def custom_performance_finite(self, x):
        '''x is an array of t_CC and brightness'''
        return - self.__objective_finite__(x)

    def optimize_performance_finite(self):
        constraint1 = {
            'type': 'eq', 'fun': lambda x: self.eps_tot - sum(x[2:5]) - self.eps_EC}
        p0 = [self.timing_imprecision[0]*3, 1e6, 99 *
              (1e-5)/100, (1e-7-1e-10)/2, (1e-7-1e-10)/2, 0.5, 0.5]
        result = minimize(self.__objective_finite__, p0, bounds=[[0, self.timing_imprecision[0]*40], [1e-5, 4e11], [
                          0, self.eps_tot], [0, self.eps_tot], [0, self.eps_tot], [1e-8, 1-1e-3], [1e-8, 1-1e-3]], constraints=[constraint1,])
